//! 8×16 像素英文字母/数字/符号点阵字库。
//!
//! 数据来源：Linux 5.4.29 `lib/fonts/font_8x16.c` 的 `fontdata_8x16` 数组。
//! 每个字符占 16 字节，每字节代表一行 8 像素（bit7=左，bit0=右）。

/// 单个字符 8×16 点阵的字节数：16 行 × (8bit/8) = 16 字节。
const CHAR_BYTES: usize = 16;

/// 完整 256 字符 × 16 字节 = 4096 字节的 8×16 点阵数据。
///
/// 索引方式：`FONT_DATA[(c as usize) * 16 .. (c as usize) * 16 + 16]`。
///
/// 参考：Linux 5.4.29 `lib/fonts/font_8x16.c:13-4096`
#[rustfmt::skip]
static FONT_DATA: [u8; 4096] = [
    // ===== 0x00-0x0f: 控制字符（大部分为空或特殊图形） =====
    ...
    // ===== 0x7f-0xff: 扩展字符/图形（从 Linux 原样复制，用于表格线等） =====
...
/// 根据 ASCII 字符返回其 8×16 点阵位图（16 字节的引用）。
///
/// 每字节代表一行像素，bit7 对应最左边像素，bit0 对应最右边像素。
/// 调用方按行遍历：`font_bitmap[row]` 取出第 `row` 行的 8 个像素位。
///
/// 非 ASCII 字符（codepoint > 0x7f）统一返回空格字符的点阵。
pub fn get_font_bitmap(c: char) -> &'static [u8; CHAR_BYTES] {
    let idx = if (c as u32) <= 0x7f {
        c as usize
    } else {
        0x20 // 非 ASCII 字符回退为空格
    };
    // 从 FONT_DATA 数组中切出该字符的 16 字节
    let base = idx * CHAR_BYTES;
    let slice: &[u8] = &FONT_DATA[base..base + CHAR_BYTES];
