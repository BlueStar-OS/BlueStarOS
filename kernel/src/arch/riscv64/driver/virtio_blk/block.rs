...
impl VirtBlk {
    ...
    pub fn new_from_dtb() -> Result<Self, &'static str> {
        ...
        Self::new_from_base(device.base)
    }

    pub fn new_from_base(base_addr: usize) -> Result<Self, &'static str> {
        unsafe {
            debug!("New VirtBlk from base at {:#x}", base_addr);
            let header = &mut *(base_addr as *mut VirtIOHeader);
...
impl Hal for VirtioHal {
    fn dma_alloc(pages: usize) -> virtio_drivers::PhysAddr {
        ...
        let base_ppn = frames.first().map(|f| f.ppn).unwrap_or(PhysiNumber(0));
