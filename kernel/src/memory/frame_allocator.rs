use log::{trace, warn};
use crate::{config::{KERNEL_HEAP_SIZE, MB, PAGE_SIZE}, sync::UPSafeCell};
            ...
            if range.current < range.end {
                ...
                unsafe {
                    core::slice::from_raw_parts_mut(
                        (ppn * PAGE_SIZE) as *mut u8, PAGE_SIZE
                    ).fill(0);
                }
                return Some(FramTracker::new(PhysiNumber(ppn)));
            }
    ...
    fn alloc_continue_frame(&mut self,page_count:usize)->Option<Vec<FramTracker>>{
        ...
        if self.active < self.ranges.len() {
            ...
            if range.remaining() >= page_count {
                ...
                unsafe {
                    core::slice::from_raw_parts_mut(
                        (ppn * PAGE_SIZE) as *mut u8, PAGE_SIZE * page_count
                    ).fill(0);
                }
        ...
        self.recycle.sort();
        let start_ppn = self.recycle.windows(page_count)
            .position(|window| {
                window[page_count - 1] - window[0] == page_count - 1
            });
    ...
    pub fn init_ranges(&mut self, regions: &[(usize, usize)]) {
        ...
        for &(start_addr, end_addr) in regions {
            ...
            if end_ppn > start_ppn {
                kprintln!("frame range: ppn [{}, {}) addr [{:#x}, {:#x}) size={}MB",
                    start_ppn, end_ppn,
                    start_addr, end_addr,
        ...
        kprintln!("frame allocator init: {} ranges, total {} pages ({}MB)",
            self.ranges.len(), total_pages, total_pages * PAGE_SIZE / MB);
    }
