...
lazy_static!{
    pub static ref KERNEL_MEMORY_SPACE_LIST:UPSafeCell<Vec<PreFlectMemory>> =UPSafeCell::new(Vec::new());
}
...
pub fn register_kernel_mmio(range: VirNumRange, flags: MapAreaFlags) {
    ...
    kprintln!("[MMIO Registry] Registers: {:#x}-{:#x}", start_addr.0, end_addr.0);
    KERNEL_MEMORY_SPACE_LIST.lock().push(PreFlectMemory::new(range, flags));
}
